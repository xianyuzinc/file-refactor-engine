...
- Scans `.tex`, `.md`, `.py`, `.ipynb`, `.json`, `.yaml`, and `.yml` files for project-local path references.
...
- LaTeX: `\includegraphics{}`, `\input{}`, `\include{}`, `\bibliography{}`, `\addbibresource{}`.
...
- moving a LaTeX figure directory and updating `\includegraphics`
...
- 扫描 `.tex`、`.md`、`.py`、`.ipynb`、`.json`、`.yaml`、`.yml` 中的项目内路径引用。
- 支持安全移动文件或文件夹。
...
- 移动 LaTeX 图片目录并更新 `\includegraphics`
- 重命名 Markdown 中引用的图片
