...
- Scans project-local path references in documents, notebooks, configs, web assets, BibTeX, and common source-code files, including MATLAB .m files.
- Supports safe `move` operations for files and directories.
- Supports batch `rename` operations with glob or regex patterns.
- Provides an interactive project tree for selecting move sources, destinations, and rename patterns.
- Builds a dry-run report before touching files.
- Shows unified diffs for text edits.
- Ships with a custom Windows app icon.
...
1. Build or download the portable executable. For a local build, run `npm run package:win`, then open:
   ...
   ```text
   release/File Refactor Engine 0.3.0.exe
   ```

2. Click **Choose Project** and select the project root folder.
3. Review the interactive project tree and detected file types.
4. Choose an operation:
   ...
   You can also use the tree actions to fill the source, destination, or rename pattern fields.
5. Click **Dry-run**.
...
- Source-code strings: Python, JavaScript, TypeScript, MATLAB .m, C/C++, Java/Kotlin/Scala, Go, Rust, C#/.NET, PHP, Ruby, Lua, Shell, R, and Julia.
- HTML and CSS: src/href-like attributes, CSS url(), and CSS @import references.
- Jupyter notebooks: markdown links and code-cell string paths; outputs are not edited.
- Config and bibliography files: JSON, YAML, TOML, and BibTeX string values that resolve to real project files.
...
- updating Python, MATLAB, JavaScript/TypeScript, HTML/CSS, JSON/YAML/TOML, and BibTeX references
...
- Context-menu and drag-and-drop tree interactions in the GUI.
- Per-edit opt-out with dependency consistency checks.
- Git status and diff integration when the project is a Git repository.
- More structured parsers for language-specific import forms and dynamic path patterns.
- Release metadata for public builds.
...
- 扫描文档、notebook、配置文件、网页资源、BibTeX，以及常见源码文件中的项目内路径引用；其中包括 MATLAB .m 文件。
- 支持安全移动文件或文件夹。
- 支持通过 glob 或 regex 批量重命名文件。
- 提供可交互项目文件树，可直接选择 move source、destination 和 rename pattern。
...
4. 选择操作：
   ...
   也可以直接用文件树上的按钮填入 source、destination 或 rename pattern。
5. 点击 **Dry-run**。
...
- Markdown：普通链接和图片链接。
- 源码字符串：Python、JavaScript、TypeScript、MATLAB .m、C/C++、Java/Kotlin/Scala、Go、Rust、C#/.NET、PHP、Ruby、Lua、Shell、R、Julia。
- HTML 和 CSS：src/href 等属性、CSS url()、CSS @import 引用。
- Jupyter notebook：markdown cell 和 code cell 里的路径；不修改 outputs。
- 配置和书目文件：JSON、YAML、TOML、BibTeX 中能解析到项目内真实文件的字符串值。
...
- 重命名 Markdown 中引用的图片
- 更新 Python、MATLAB、JavaScript/TypeScript、HTML/CSS、JSON/YAML/TOML、BibTeX 引用
...
- 增加更结构化的语言专用 import 解析和动态路径模式识别。
- 增加公开发布元数据。
- 公开发布前增加签名安装包。
